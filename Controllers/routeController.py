...
class RouteControl:
    ...
    def recover_route(self, email: str) -> tuple:
        ...
        if not mc.send_mail(email, "Recuperação de Conta Livro para Todxs", f"Olá {ac.decrypt(get_user_by_email[0]['first_name'], config.CRYPT_KEY)}, copie o código e cole no seu navegador.\n\n{token['token_id']}\n\nLembrando que seu link é valido por 15 minutos!"):
    ...
    def set_address_route(self, address_data: dict, user_id: str) -> tuple:
